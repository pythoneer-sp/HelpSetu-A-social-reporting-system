...
def send_email(r_email,new_report,file_path):
    ...
    body = (
        f"New Report has been added:-\n\n"
        f"Details:\n"
        f"Victim's Name - {new_report['victim_name']}\n"
        f"Issue type - {new_report['issue_type']}\n"
    ...
    if 'latitude' in new_report and 'longitude' in new_report:
        latitude = new_report['latitude']
        longitude = new_report['longitude']
        google_maps_url = f"https://www.google.com/maps/?q={latitude},{longitude}"
        body += f"Location: {google_maps_url}\n"
    ...
    if file_path:
        ...
        if mime_type:
            ...
            with app.open_resource(file_path) as fp:
                ...
                msg.attach(file_name, mime_type, file_content)
    # Sends the mail
    mail.send(msg)
...
StateReport = StateReportModel(db)

def increment_report_count(db, submitted_state):
    # Query the database for the existing report count for the state
    state_report = db.session.query(StateReport).filter_by(state_name=submitted_state).first()

    # If the state does not exist in the database, create a new record
    if not state_report:
        state_report = StateReport(state_name=submitted_state, report_count=1)
        db.session.add(state_report)
    else:
        # If it exists, increment the report count
        state_report.report_count += 1
    db.session.commit()
...
def sendus():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            new_report = Report(name=name, contact=contact, victim_name=victim_name,issue_type=issue_type,
            ...
            db.session.commit()
            # Increment the report count for the state
            increment_report_count(db,state)
    ...
    return render_template('sendus.html',issue_type=issue_type)
...
def trackreport():
    # Query the database for report counts per state
    report_counts = db.session.query(Report.state, func.count(Report.state)).group_by(Report.state).all()
    report_data = {state: count for state, count in report_counts}
    return render_template('map.html', report_data=report_data)
